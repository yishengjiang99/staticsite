...
export async function initsfbk(url, port) {
    const ab = await (await fetch(url, { mode: 'no-cors', headers: { Range: 'bytes=0-6400' } })).arrayBuffer();
    ...
    const parr = new Uint8Array(await (await fetch(url, {
...
export function getSampleIzones(sid, pdta) {
    return pdta.ibag
