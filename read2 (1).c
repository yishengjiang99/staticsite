...
void loop(voice *v, channel_t *ch)
{
	...
	for (int i = 0; i < 128; i++)
	{
		...
		float mono = gain * centdbLUT[(int)envShift(v->ampvol)] * centdbLUT[v->z->Attenuation];
		outputInterweaved[2 * i] = mono * (1 + pan / 1000) / 3;
...
{
	readsf("file.sf2");
	shdrcast *c = (shdrcast *)shdrs;

	printf("%d", c->originalPitch);
	FILE *proc = popen("ffplay -i pipe:0 -f f32le -ac 2 -ar 48k", "w");
	//	FILE *proc = popen("od -f", "w");

	init_ctx();
	initLUTs();
	(ctx->channels + 0)->program_number = 0;
	(ctx->channels + 1)->program_number = 55;
	(ctx->channels + 2)->program_number = 2;

	noteOn(0, 55, 44);
	noteOn(1, 57, 55);
	noteOn(2, 59, 55);
	render(.25 * 48000, proc);
	noteOff(0, 55);
	noteOff(1, 57);
	noteOff(2, 59);
	render(.5 * 48000, proc);
